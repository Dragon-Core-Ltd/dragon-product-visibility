...
class Ajax {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_dpv_search_customers', [$this, 'search_customers']);

        // Save visibility rules AJAX
        add_action('wp_ajax_dpv_save_visibility_rules', [$this, 'save_visibility_rules']);

        // Get visibility rules AJAX
        add_action('wp_ajax_dpv_get_visibility_rules', [$this, 'get_visibility_rules']);
    }
    ...
    public function search_customers(): void {
        ...
        if (!isset($_REQUEST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'])), 'dpv_admin_nonce')) {
            wp_send_json_error(['message' => __('Security check failed', 'dragon-product-visibility')]);
            return;
        }

        // Check permissions
        if (!current_user_can('edit_products')) {
            wp_send_json_error(['message' => __('Permission denied', 'dragon-product-visibility')]);
            return;
        }
    ...
    public function save_visibility_rules(): void {
        ...
        if (!$product_id) {
            wp_send_json_error(['message' => __('Invalid product ID', 'dragon-product-visibility')]);
            return;
        }
        ...
        $role_ids = isset($_POST['role_ids']) ? array_map('sanitize_text_field', (array) $_POST['role_ids']) : [];
        ...
        wp_send_json_success(['message' => __('Visibility rules saved', 'dragon-product-visibility')]);
    }
